run_data_update <- function() {
  ...
  attendance_data_raw <- sqlQuery(conn, "SELECT * FROM school_attendance_national_stats.aut_2425_oa_clean_delta")
...
process_attendance_data <- function(attendance_data_raw, start_date, end_date, pa_fullyear_file) {
  ...
  attendance_data_daily <- attendance_data %>%
    mutate(
      ...
      # excluded_perc = (reason_e_authorised_excluded / possible_sessions) * 100, #removed as is a duplicate
      ...
      auth_mob_perc = (reason_t_authorised_mobile_child / possible_sessions) * 100,
      ...
      auth_performance_perc = (reason_c1_authorised_regulated_performance / possible_sessions) * 100,
      auth_interview_perc = (reason_j1_authorised_interview / possible_sessions) * 100,
      auth_part_time_perc = (reason_c2_authorised_temp_reduced_timetable / possible_sessions) * 100,
  ...
  attendance_data_ytd <- attendance_data %>%
    ...
    mutate(
      ...
      # time_identifier = 37, #removed as don't think this is needed for ytd
      num_schools = mean(num_schools),
      enrolments = mean(enrolments),
      present_sessions = sum(present_sessions),
      overall_attendance = sum(overall_attendance),
  ...
  attendance_data <- attendance_data %>%
    mutate_at(vars(
      ...
      auth_other_perc,
      # pa_flag,
      # ytd_enrolments, #commented out because associated with PA
      # pa_perc,
