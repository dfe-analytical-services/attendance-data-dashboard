...
attendance_data_raw <- attendance_data_raw %>% filter(
  time_identifier != "Week 52",
  region_name != "null",
  new_la_code != "E06000048"
)
...
# Calculate date
attendance_data <- attendance_data %>% mutate(attendance_date = as.Date(as.character(attendance_date), format = "%d/%m/%y"))
...
attendance_data_daily <- attendance_data %>%
  mutate(
    attendance_perc = (present_sessions / possible_sessions) * 100,
...
attendance_data_weekly_totals <- attendance_data %>%
  ...
  summarise(across(matches("attendance_date"), min, na.rm = T),
    ...
    across(where(is.numeric) & !c(attendance_date, day_number), sum),
    na.rm = T
  ) %>%
  mutate(
    school_type = "Total",
...
attendance_data_ytd_totals <- attendance_data %>%
  ...
  summarise(across(matches("time_identifier"), min, na.rm = T),
    ...
    across(matches("day_number"), min, na.rm = T),
    across(where(is.numeric) & !c(time_identifier, attendance_date, day_number), sum),
    na.rm = T
  ) %>%
  mutate(
    school_type = "Total",
...
attendance_data_fordownload <- attendance_data %>%
  dplyr::select(
    time_period,
    time_identifier,
    geographic_level,
...
mapshape <- st_read("reference/CTYUA_DEC_2021_UK_BUC.shp") %>% st_transform(crs = 4326)
